function [indices,f,pxx] = CalcIndices(signal,t,peaks_ind,psd_est)
indices=cell(2,10);
L=length(signal);
...
indices{1,5}='TP';
indices{2,5}=trapz(f([vlf_ind; lf_ind; hf_ind]),pxx([vlf_ind; lf_ind; hf_ind]));
%sum(pxx([vlf_ind; lf_ind; hf_ind]))/length(pxx);
indices{1,6}='HF';
indices{2,6}=trapz(f(hf_ind),pxx(hf_ind));
%sum(pxx(hf_ind))/length(pxx);
indices{1,7}='LF';
indices{2,7}=trapz(f(lf_ind),pxx(lf_ind));
